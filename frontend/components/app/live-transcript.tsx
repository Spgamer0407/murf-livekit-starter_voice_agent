...
export function LiveTranscript() {
  ...
  return (
    <div className="glass-panel flex h-full min-h-0 w-full flex-col overflow-hidden rounded-[2rem]">
      <div className="flex shrink-0 items-center gap-2 border-b border-slate-200/50 bg-white/50 px-6 py-5 backdrop-blur-md dark:border-slate-800/50 dark:bg-slate-900/50">
        <MessageSquare className="size-5 text-indigo-600 dark:text-indigo-500" />
        <h3 className="font-sans text-sm font-bold tracking-wider text-slate-700 uppercase dark:text-slate-300">
          Live Transcript
        </h3>
      </div>
          ...
          <AnimatePresence initial={false}>
            {messages.length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mt-12 flex h-full flex-col items-center justify-center text-center text-slate-400"
              >
              ...
              messages.map((msg) => (
                ...
                >
                  <span className="mb-1.5 px-1 font-sans text-[11px] font-bold tracking-wider text-slate-400 uppercase">
                    {msg.isAgent ? 'Shiksha AI' : 'You'}
                  </span>
                  <div
                    className={`max-w-[85%] rounded-2xl px-5 py-3.5 shadow-sm ${
                      msg.isAgent
                        ? 'rounded-tl-sm border border-slate-200/50 bg-white text-slate-800 dark:border-slate-700/50 dark:bg-slate-800 dark:text-slate-200'
                        : 'rounded-tr-sm bg-indigo-600 text-white shadow-md shadow-indigo-900/10 dark:bg-indigo-700'
