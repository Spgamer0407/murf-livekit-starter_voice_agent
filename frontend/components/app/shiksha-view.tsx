...
export function ShikshaView({ startButtonText, onStartCall }: ShikshaViewProps) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col transition-colors duration-700">
      ...
      <main className="relative z-10 flex min-h-0 flex-1 items-center justify-center p-4 md:p-8">
        <AnimatePresence mode="wait">
          ...
          {isReady && (
            ...
            >
              <div className="flex flex-1 flex-col items-center text-center lg:items-start lg:text-left">
                <motion.h2 className="mb-4 font-serif text-5xl leading-tight font-bold text-slate-800 md:text-6xl dark:text-slate-100">
                  Speak Naturally,
                  <br />
                  <span className="text-gradient">Learn Confidently.</span>
                </motion.h2>

                <motion.p className="mb-10 max-w-lg font-sans text-lg text-slate-600 dark:text-slate-400">
              ...
              <div className="relative flex flex-1 items-center justify-center">
                <div className="relative flex h-72 w-72 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500/10 to-indigo-700/10 md:h-96 md:w-96">
                  <div className="absolute inset-0 animate-[spin_10s_linear_infinite] rounded-full border border-indigo-500/20" />
                  <div className="absolute inset-4 animate-[spin_15s_linear_infinite_reverse] rounded-full border border-indigo-600/20" />
                  <div className="z-10 size-48 overflow-hidden rounded-full border-4 border-white/50 bg-slate-100 shadow-2xl md:size-64 dark:border-slate-800/50 dark:bg-slate-900">
                    <Image
          ...
          {isConnecting && (
            ...
            >
              <div className="relative flex size-40 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500/20 to-indigo-700/20 shadow-2xl">
                <div className="absolute inset-0 animate-ping rounded-full bg-indigo-500/20" />
                <Image
                  src="/images/human-avatar.png"
                  width={144}
                  height={144}
                  className="z-10 size-full rounded-full border-4 border-white/50 object-cover dark:border-slate-800/50"
                  alt="Avatar"
                />
              </div>
              <p className="mt-8 font-serif text-2xl font-bold text-slate-800 dark:text-slate-200">
                Waking up Shiksha...
              </p>
              <p className="mt-2 font-medium text-slate-500">Please wait a moment</p>
            </motion.div>
          )}
          ...
          {isConnected && !isConnecting && (
            ...
            >
              ...
              <div className="glass-panel relative flex min-h-0 flex-col items-center rounded-3xl p-6 lg:p-8">
                <div className="absolute top-6 right-6 flex items-center gap-2 rounded-full bg-slate-100/50 px-4 py-2 text-sm font-bold shadow-sm backdrop-blur-md dark:bg-slate-900/50">
                ...
                <div className="relative mt-2 mb-4 shrink-0">
                  ...
                  />
                  <div className="relative flex size-32 items-center justify-center overflow-hidden rounded-full border-4 border-white/50 bg-slate-100 shadow-xl md:size-40 dark:border-slate-800/50 dark:bg-slate-900">
                    <Image
                      src="/images/human-avatar.png"
                      alt="Coach Avatar"
                      width={160}
                ...
                <motion.div
                  ...
                  className={`mb-4 shrink-0 rounded-full px-6 py-2 font-sans text-sm font-bold tracking-wider uppercase shadow-sm transition-colors ${
                    agentState === 'speaking'
                      ? 'border border-indigo-200 bg-indigo-100 text-indigo-700 dark:border-indigo-800/50 dark:bg-indigo-900/40 dark:text-indigo-300'
                ...
                <div className="mb-6 flex min-h-0 w-full shrink items-center justify-center rounded-2xl bg-slate-50/50 p-4 shadow-inner ring-1 ring-slate-900/5 dark:bg-slate-950/50 dark:ring-white/5">
                  <ShikshaVisualizer />
                </div>
                ...
                <motion.div className="mt-auto flex w-full shrink-0 items-center justify-center gap-4">
                  <Button
                    ...
                    className={`group h-14 min-h-[48px] flex-1 rounded-xl px-6 font-sans text-base font-bold shadow-sm transition-all ${
                      isMicEnabled
                        ? 'border-2 border-slate-200 bg-white/50 text-slate-700 hover:bg-white hover:text-slate-900 dark:border-slate-700 dark:bg-slate-900/50 dark:text-slate-300 dark:hover:bg-slate-800'
                  ...
                  <Button
                    ...
                    size="lg"
                    onClick={() => end()}
                    className="group h-14 min-h-[48px] flex-1 rounded-xl border-2 border-slate-200 px-8 font-sans text-base font-bold text-slate-700 transition-colors hover:border-red-500 hover:bg-red-50 hover:text-red-700 dark:border-slate-700 dark:text-slate-300 dark:hover:border-red-800 dark:hover:bg-red-950/50 dark:hover:text-red-400"
                  >
          ...
          {isSessionEnded && (
            ...
            >
              <h2 className="mb-2 font-serif text-4xl font-bold text-slate-800 dark:text-slate-100">
                Session Complete
              </h2>
              <p className="mb-8 font-sans text-lg text-slate-600 dark:text-slate-400">
                Great job practicing today!
              </p>
              <div className="mb-8 grid w-full grid-cols-2 gap-4">
                <div className="rounded-2xl border border-slate-200/50 bg-white/40 p-6 dark:border-slate-800/50 dark:bg-slate-900/40">
                  <Clock className="mx-auto mb-2 size-8 text-indigo-500" />
                  <p className="font-sans text-3xl font-bold text-slate-800 dark:text-slate-200">
                    {formatTime(sessionSeconds)}
                  </p>
                  <p className="mt-1 font-sans text-xs font-bold tracking-wider text-slate-500 uppercase">
                    Duration
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200/50 bg-white/40 p-6 dark:border-slate-800/50 dark:bg-slate-900/40">
      ...
      <footer className="relative z-10 flex items-center justify-center py-4 font-sans text-xs font-semibold text-slate-400 dark:text-slate-600">
